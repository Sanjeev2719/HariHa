...
function ProjectCardTile({ project, idx, onSelect, isPaused }: { project: Project; idx: number; onSelect: (project: Project, startWithVideo?: boolean) => void; isPaused?: boolean; key?: string }) {
  ...
  return (
    ...
    >
      ...
      {/* Lightweight Optimized Image or Google Drive Cover Container */}
      <div className="aspect-4/3 w-full overflow-hidden relative rounded-t-3xl bg-gradient-to-br from-[#0F172A] via-[#1E293B] to-[#0F172A] flex items-center justify-center">
        {images.length > 0 ? (
          <>
            {images.map((imgUrl, i) => (
              <motion.img
          ...
          <div className="relative z-10 p-6 text-center flex flex-col items-center justify-center space-y-3">
            <div className="w-14 h-14 rounded-2xl bg-[#0052FF]/20 border border-[#0052FF]/40 flex items-center justify-center text-[#00D2FF] shadow-[0_0_25px_rgba(0,82,255,0.35)] group-hover:scale-110 transition-transform duration-300">
              <FolderOpen size={28} />
            </div>
            <div>
              <span className="text-[10px] font-sans font-bold uppercase tracking-[0.2em] text-[#00D2FF] block mb-1">
                Google Drive Showcase
              </span>
              <p className="text-xs text-white/80 font-light max-w-xs leading-relaxed">
                Watch Site Walkthrough &amp; Video Footage Hosted on Google Drive
              </p>
            </div>
            {project.driveVideoUrl && (
              <a
                href={project.driveVideoUrl}
                target="_blank"
                rel="noopener noreferrer"
                onClick={(e) => e.stopPropagation()}
                className="inline-flex items-center space-x-2 bg-[#0052FF] hover:bg-[#0040D0] text-white text-[10px] font-bold uppercase tracking-widest px-4 py-2 rounded-full shadow-lg border border-white/20 transition-all hover:scale-105 mt-1"
              >
        ...
        <div className="absolute inset-0 p-6 flex flex-col justify-between opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-10 bg-[#0F172A]/70 backdrop-blur-sm">
        ...
        <div className="flex items-center justify-between">
          ...
          {project.driveVideoUrl ? (
            <a
              ...
              rel="noopener noreferrer"
              onClick={(e) => e.stopPropagation()}
              className="text-[9px] uppercase tracking-wider text-[#0052FF] font-bold flex items-center space-x-1 hover:underline"
            >
  ...
  return (
    <>
      ...
      {typeof document !== 'undefined' && createPortal(
        <AnimatePresence>
          {selectedProject && (() => {
            const projectImages = (selectedProject.images || []).filter(Boolean);
            if (projectImages.length === 0 && selectedProject.image) {
              projectImages.push(selectedProject.image);
            }
            const driveEmbedUrl = getDriveEmbedUrl(selectedProject.driveVideoUrl);
            const isVideoSlideActive = driveEmbedUrl !== null && (activeLightboxImg === projectImages.length || projectImages.length === 0);
            const totalSlides = driveEmbedUrl ? (projectImages.length === 0 ? 1 : projectImages.length + 1) : projectImages.length;
